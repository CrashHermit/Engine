  ...
  roll_gate
    ...
    └─(roll)──► segmenter ── {lead_up, contested_beat, deferred_tail}
                    ...
                    └──► ┌─ attribute_selector ──┐
                         ...
                         ├─ threat_magnitude ─────┤                    │            │
                         └─ threat_channel ────────┘               avoided      landed ≥ MINOR
                                                                       │              │
...
- **Significance gate** (decision #33, *revised*): `landed_magnitude ≥ Minor (1)` → `held_planner`;
  `landed_magnitude == 0` (avoided — clean/crit) → `final_planner` directly. Any consequence
  that actually lands is resistible; only avoided results skip the resist offer. *(Originally
  gated at `≥ Standard`, but with lean pools (#8) a reduced roll demotes Standard→Minor and
  suppressed the offer on the engine's intended hot path; gating on "anything landed" restores
  Blades-faithful resist-anything and makes the resistance pipeline the hot path as #8 expects.)*
...
| *significance gate* | `landed_magnitude ≥ Minor (1)` → held path; `0` (avoided) → final path | routing (code, no LLM — decision #33, revised) |
...
| **held-planner** (any landed consequence, ≥ Minor) | extract the held-state scaffold so the narrator can commit to impact while leaving depth/finality unresolved | structured — `HeldScaffold = {impact_focus, sensory_anchors: list, ambiguity_wedge: list, tension_close, resist_options_text}` (decision #33) |
