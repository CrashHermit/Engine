#!/usr/bin/env python3
"""Generate 5 worlds with 50 plates, 75% oceanic."""
...
def main() -> None:
    output_base = Path("output/50plate_75oceanic")
    seeds = [301, 302, 303, 304, 305]
    size = 100
    resolution = 512
    n_plates = 50
    oceanic_fraction = 0.75  # 75% oceanic
    continental_fraction = 1.0 - oceanic_fraction

    print(f"Generating {len(seeds)} worlds with {n_plates} plates, 75% oceanic...\n")
    print(f"Configuration per world:")
    print(f"  Continental plates: ~{int(n_plates * continental_fraction)} (30%)")
    print(f"  Oceanic plates: ~{int(n_plates * oceanic_fraction)} (75%)\n")

    for i, seed in enumerate(seeds, 1):
        seed_dir = output_base / f"seed_{seed}_50plates_75oceanic"
    ...
    print(f"\n{'='*60}")
    print(f"✓ Generated {n_plates}-plate, 75% oceanic worlds")
    print(f"{'='*60}")
    print(f"Seeds: {', '.join(str(s) for s in seeds)}")
    print(f"Configuration: {n_plates} plates, {int(n_plates * continental_fraction)} continental, {int(n_plates * oceanic_fraction)} oceanic")
    print(f"Result: Water-dominated worlds with scattered small landmasses")
