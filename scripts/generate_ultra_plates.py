#!/usr/bin/env python3
"""Generate a single world with 60 tectonic plates."""
...
def main() -> None:
    output_base = Path("output/ultra_plates")
    seed = 42
    size = 100
    resolution = 512
    n_plates = 60

    print(f"Generating world with {n_plates} plates (seed {seed})...\n")

    output_dir = output_base / f"seed_{seed}_60plates"
    ...
    print(f"\n{'='*60}")
    print(f"✓ Generated 60-plate world in {output_dir}")
    print(f"{'='*60}")
    print(f"Seed: {seed}")
    print(f"Plates: {n_plates}")
    print(f"Extreme plate fragmentation - highly complex terrain!")
