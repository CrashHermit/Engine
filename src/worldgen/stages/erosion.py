...
class ErosionStage:
    ...
    * **Thermal erosion**: elevation is diffused toward the talus angle
    ...
    def run(self, ctx: WorldContext) -> WorldContext:
        if not self._config.enabled or ctx.data.mesh is None:
            return ctx
        ...
        cells = mesh.cells
        sorted_z = sorted(cell.z for cell in cells)
    ...
    def _flow_and_flux(
        ...
        for cell in cells:
            ...
            for nid in cell.neighbors:
                if cells[nid].z < best_z:
                    best_z = cells[nid].z
                    best = nid
            flow[cell.id] = best

        land_cells = sorted(
            (c for c in cells if c.is_land), key=lambda c: c.z, reverse=True
        )
        flux = [0.0] * len(cells)
        for cell in land_cells:
            flux[cell.id] += 1.0
            ds = flow.get(cell.id)
            if ds is not None and cells[ds].is_land:
    ...
    def _stream_power(
        ...
        for cell in cells:
            ...
            if ds is None:
                continue
            slope = max(0.0, cell.z - cells[ds].z)
            erosion = k * (flux[cell.id] ** 0.5) * slope * 0.001
            cell.z = max(0.0, cell.z - erosion)
