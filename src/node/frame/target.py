...
class TargetSignature(Signature):
    """
    Given the contested beat the player is about to roll, identify the target:
    the entity the action is directed at (the foe being struck, the lock being
    forced). Return its exact name from the listed entities, or an empty string
    if the action targets no specific entity (movement, perception, environment).
    """
...
class TargetNode:
    ...
    async def __call__(self, state: GraphState) -> dict:
        candidates = state.scene_entities
        if len(candidates) <= 1:
