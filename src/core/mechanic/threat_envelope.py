...
def snap_channel(predicted: Channel, affinity: frozenset[Channel]) -> Channel:
    """Anchor the classifier's channel into the source's affinity (belt-and-
    suspenders). Empty affinity (environment) leaves the prediction untouched."""
...
def describe_threat(threat: Threat | None) -> str:
    """One-line consequence label used in offers, parser context, and
    ...
    return f"{mag_label} {threat.type.value} ({threat.channel.value}) from the {threat.source}"