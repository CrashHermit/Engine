...
def test_known_tick_values(unit, expected):
    ...
    assert TICKS[unit] == Span(unit).ticks


def test_coarse_span_ticks_multiply_by_count():
    assert Span(Unit.WEEK, 3).ticks == 3 * 100_800
    assert Span(Unit.MONTH, 5).ticks == 5 * 432_000
...
def test_coarse_unit_allows_counts_up_to_cap(unit):
    assert Span(unit, MAX_COUNT).ticks == MAX_COUNT * TICKS[unit]
    with pytest.raises(ValueError):
        Span(unit, MAX_COUNT + 1)
...
def test_three_weeks_keeps_resolution():
    ...
    assert span_from_ticks(Span(Unit.WEEK, 3).ticks) == Span(Unit.WEEK, 3)


def test_overflowing_count_steps_up_a_unit():
    # 10 weeks overflows the Week cap -> rolls up to ~2 months.
    assert normalize(Unit.WEEK, 10) == Span(Unit.MONTH, 2)


def test_five_months_round_trips():
    assert normalize(Unit.MONTH, 5) == Span(Unit.MONTH, 5)


def test_few_days_stay_in_days():
    assert span_from_ticks(Span(Unit.DAY, 4).ticks) == Span(Unit.DAY, 4)
...
def test_exact_fine_rung_round_trips(unit):
    assert span_from_ticks(TICKS[unit]) == Span(unit, 1)


def test_huge_total_clamps_at_year_ceiling():
    assert span_from_ticks(10**12) == Span(Unit.YEAR, MAX_COUNT)


def test_sub_round_floors_to_round():
    assert span_from_ticks(0) == Span(Unit.ROUND, 1)
